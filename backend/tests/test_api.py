...
class TestSenatorAPI:
    ...
    def test_get_senator_by_id_existing(self):
        ...
        if senators:
            ...
            response = requests.get(f"{BASE_URL}/api/senators/{senator_id}", timeout=TIMEOUT)
    ...
    def test_senators_data_quality(self):
        ...
        for senator in senators:
            ...
            assert senator["state"].isupper(), f"State should be uppercase: {senator['state']}"
...
class TestRepresentativeAPI:
    ...
    def test_get_representative_by_id_existing(self):
        ...
        if reps:
            rep_id = reps[0]["id"]
            response = requests.get(f"{BASE_URL}/api/representatives/{rep_id}", timeout=TIMEOUT)
    ...
    def test_get_representative_by_id_nonexistent(self):
        ...
        response = requests.get(f"{BASE_URL}/api/representatives/99999", timeout=TIMEOUT)
        assert response.status_code == 404
    ...
    def test_representatives_data_quality(self):
        ...
        for rep in reps:
            ...
            assert isinstance(rep["district"], int), f"District should be integer: {rep['district']}"
            assert rep["district"] >= 0, f"District should be positive: {rep['district']}"
